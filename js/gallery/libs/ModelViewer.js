 ...
 *  Adapted for the Compliance Website by the Compliance Dev Team
 */
...
function ModelViewer(container) {
	...
	this.element.setAttribute('class', 'position: absolute; top: 0; bottom: 0; left: 0; right: 0; width: auto; height: auto; overflow: hidden;')

	this.container.appendChild(this.element)

	// get element dimensions - MODIFIED
	var rect = {width: 512, height: 512};
	...
	// draw
	this.draw = function() { self.renderer.render(self.scene, self.camera) }
...
function JsonModel(name, rawModel, texturesReference, clipUVs) {
	...
	// parse model or throw an error if parsing fails - MODIFIED -> directly use parsed json
	try {
	...
	references.forEach(function(ref, index) {
		...
		if (!urlExists(image)) image = '../../image/gallery/not-found.png';
